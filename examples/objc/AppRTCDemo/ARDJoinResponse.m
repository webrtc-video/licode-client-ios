...
static NSString const *kARDJoinSecureKey = @"secure";
static NSString const *kARDJoinTokenIdKey = @"tokenId";
static NSString const *kARDJoinHostKey = @"host";
...
+ (ARDJoinResponse *)responseFromJSONData:(NSData *)data {
  ...
  NSDictionary *responseJSON = [NSDictionary dictionaryWithJSONData:decodedData];
  if (!responseJSON) {
    return nil;
  }
  ARDJoinResponse *response = [[ARDJoinResponse alloc] init];
    response.isSecure = [responseJSON[kARDJoinSecureKey] boolValue];
    response.tokenId = responseJSON[kARDJoinTokenIdKey];
    response.host = responseJSON[kARDJoinHostKey];
    response.tokenDict = responseJSON;
